...
#        PS1: 这个脚本参考过shimmer之前写的
#        PS2: 这个工具是jameyli在TNT中写出的，当时基于我写的一些解析xls脚本。

#   History:
2013年8月14日 19:36:20 kevin -> 看了一下shimmer写的转换脚本，在做商城时，觉得欠缺一点功能
            如需要定义N种物品，即需要表格支持结构体的定义和重复N次。于是打算添加点功能。

2013年8月15日 18:10:58  kevin -> 本来想基于原脚本修改的，现在几乎是重写了遍解析过程。不想看原代码了。
            为什么看别人的代码总那么难受呢，是因为没有注释，还是没有耐心？？
            当然，还是要感谢前人的代码作参考~~~~

2013年8月15日 22:27:06 kevin
添加了对默认值的支持。当为空时，使用默认值。有人说默认值会加大表格占内存，试问有多少会是为空的？？空太多说明表格设计就有问题吧？？

2013年8月16日 11:50:21 kevin
添加带级别的标准输出？ 和打印日志类似。如设置级别高时，只打印特别重要的日志，当级别低时，一些调试的日志会打印出来
当脚本解析出问题时，调整日志级别以便更好的定位问题

2013年8月21日 22:21:20 kevin
将通过这个导出的配置统一在模块config下。其实不用模块更好 。直接全局可见？

2013年8月28日 12:48:32 kevin
添加对表格中有重复的key的解析。在使用脚本时通过参数指定是否有此行为

2013年9月13日 16:13:15 kevin
将这个解析模块独立出来。main拆在别的文件中实现

2013年9月5日 19:37:25  kevin
添加支持subkey.当key有重复时，可以指定是否有sub_key, 如果设置sub_key_col为非0.则认为那行是sub key的值。
具体的应用是表格Dungeon.xls中的DUNGEON_LIST表格。
命令：./excel2lua_kevin.py excel/Dungeon2.xls DUNGEON_LIST 1 1
说明： 第一个参数1表示这个表格的key是有重复的。 第二个1表示在处理重复的key时，对应table的sub_key为第1列的值(从0开始)

2013年11月8日 13:29:58 kevin
将table中的索引从1开始（之前从0，不太符合lua的数组规则)

2013年11月14日 16:21:36 kevin
支持表格的key为string。生成的table形式如 ["宋江"] = {70000100, 70000101}

2013年12月2日 17:10:15 kevin
对于array数组解析，自动去除多余的分隔符（在文本末尾的分隔符,如123;355;这里的最后那个分号;）

2013年12月19日 17:07:59 kevin
添加自行指定分隔符功能，在array后添加=, 或者=|这些，指定分隔符为,或者|

2014年1月7日 15:10:51 kevin
修改digit array/string array中当value为空时，生成失败的问题

2014年1月8日 20:41:59 kevin
修改digit类型的值，使不会生成浮点数值。我们不会有浮点数。全部转换为整数
修改对于array类型，生成时不使用默认值，不然会有{0}这种非预期的输出

2014年1月21日 16:47:41 kevin
应客户端同学(garyzheng)的要求，并且在客户端内存占用的对比过后，认为游戏中的配置占用内存太高，这里将配置生成的空项去除，
听说这样能减少建表的时间.我想应该会吧。 满足你们~~~ 使用defaultValue = "nil" 作为是否置空的标志。

2014年1月23日 00:05:17 kevin
修复上面修改造成的bug.有些空的struct地方需要跳过生成

2014年01月23日 15:33:06 shimmer
过年还想回家了，现在这个默认值的问题导致服务器老是coredown，先简单暴力地把svr与cli生成配置的地方分成两份把

2014年3月17日 15:24:55 @kevin
应客户端要求(armyshu), 为了能够打配置的差异包，将生成的文件的头部的时间修改为那个文件的最后更新时间,防止影响差异包生成

2014年6月19日 13:15:47 @kevin
应客户端要求，将客户端生成的配置，去掉date那一行。否则说会影响他们的差异包。。

2014-12-20 14:03:01 @kevin
重构此模块，将代码写得易懂些
...
# LastChange: 2013-08-14 19:31:09
#========================================================================
'''
import os
import xlrd
...
class CardInterpreter:
    ...
    def _GetColHeader(self, col):
        ...
        #校验一下合法性
        vaild_types = [u"string", u"digit", u"time", u"string array", u"digit array", u"struct"]
        if colName == "":
    ...
    def _ParseHeaderInfo(self, svr_or_client):
        ...
        while col < self._col_count:
            ...
            if svr_or_client == "svr" and header_item.outType == "C":
            ...
            if header_item.valueType == "struct":
                ...
                while struct_idx <  idx_max:
                    ...
                    if svr_or_client == "svr" and item.outType == "C":
                        logging.debug("this is client config, server no need")
                        pass
    ...
    def _ReadContextFromSheet(self, svr_or_client):
        ...
        context_result = ""
        #打印最前面的一些信息
        context_result += self._sheet_name + " = {\n"
        ...
        for row in range(4, self._row_count): #从第4行开始是因为前三行被用来定义表结构了
            ...
            if self._have_repeated_key == 1:
                ...
                logging.debug("self._sub_key_col:%d|repeated_key_inside:%d"%(self._sub_key_col, repeated_key_inside))
    ...
    def _GetItemString(self, header_item, row, svr_or_client):
        ...
        if header_item.valueType == "digit": #解析数字
            return_str += header_item.colName + '=' + self._Conv2IntStr(value) + ', '
        elif header_item.valueType == "string": #解析字符串
            strip_new_line = unicode(value).replace("\n","")
            return_str +=  header_item.colName + '= "' + strip_new_line + '", '
        elif header_item.valueType == "time":
            if str(value) == "":
                return_str += header_item.colName + '=0, '
            else:
                return_str += header_item.colName + '=' + str(int(time.mktime(time.strptime(str(value), "%Y-%m-%d %X")))) + ', '
        elif header_item.valueType == "digit array"  or header_item.valueType == "string array": #数字数组
            return_str += header_item.colName + '= {'
